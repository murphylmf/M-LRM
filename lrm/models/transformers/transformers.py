...
from lrm.models.transformers.custom_attention import CustomAttention, SparseAttnProcessor, XFormersSparseAttnProcessor
...
class BasicTransformerBlock(nn.Module, MemoryEfficientAttentionMixin):
    ...
    def __init__(
        ...
        num_attention_heads_cross: int,
        attention_head_dim_cross: int,
        dropout=0.0,
        ...
        self.apply_sparse_attention = apply_sparse_attention
        ...
        if cross_attention_dim is not None or double_self_attention:
            ...
            if apply_sparse_attention:
                # use our custom attention
                self.attn2 = CustomAttention(
                    query_dim=dim,
                    cross_attention_dim=cross_attention_dim
                    if not double_self_attention
                    else None,
                    heads=num_attention_heads_cross,
                    dim_head=attention_head_dim_cross,
                    dropout=dropout,
                    bias=attention_bias,
                    upcast_attention=upcast_attention,
    ...
    def forward(
        ...
        kv_index: Optional[torch.LongTensor] = None,  # [B 3p V V (V N) 3c]
        dino_shape: Optional[torch.LongTensor] = None, # [ViewNum H W]
    ) -> torch.FloatTensor:
        ...
        if self.attn2 is not None:
            ...
            if self.apply_sparse_attention:
...
class Transformer1D(BaseModule, MemoryEfficientAttentionMixin):
    ...
    class Config(BaseModule.Config):
        ...
        num_attention_heads_cross: Optional[int] = None
        attention_head_dim_cross: Optional[int] = None
    ...
    def configure(self) -> None:
        ...
        self.num_attention_heads_cross = self.cfg.num_attention_heads_cross \
            if self.cfg.cross_attention_dim is not None else self.num_attention_heads
        self.attention_head_dim_cross = self.cfg.attention_head_dim_cross \
            if self.cfg.cross_attention_dim is not None else self.attention_head_dim
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                BasicTransformerBlock(
                    ...
                    self.num_attention_heads_cross,
                    self.attention_head_dim_cross,
                    ...
                    apply_sparse_attention=self.cfg.apply_sparse_attention,
                )
    ...
    def forward(
        ...
        for i, block in enumerate(self.transformer_blocks):
            if self.cfg.apply_sparse_attention and self.cfg.interleave and i % 2 == 0:
